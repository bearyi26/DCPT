...
def index_to_color(idx):
    return {
        ...
        3: cv_colors.PURPLE.value,
        4: cv_colors.YELLOW.value,
        5: cv_colors.MINT.value,
        6: cv_colors.WHITE.value,
    }[idx]
...
class VisTracking(VisBase):
    def __init__(self, visdom, show_data, title):
        super().__init__(visdom, show_data, title)
        self.count = 0
        videopath = '/home/apple/ICRA2024/DCPT/vis/video-nat2021/'
        video = '/home/apple/ICRA2024/DCPT/vis/video-nat2021/video_N01010.mp4'
        import os
        try:
            os.mkdir(videopath)
        except:
        ...
        self.videowriter = cv2.VideoWriter(video, fourcc, 25, size)  # 25是每秒的帧数，size是图片尺寸
    ...
    def draw_data(self):
        ...
        for i, disp_rect in enumerate(boxes):
            ...
            try:
                cv2.rectangle(disp_image,
                          ...
                          (int(disp_rect[0] + disp_rect[2]), int(disp_rect[1] + disp_rect[3])), color, 3)
            except:
                continue
        ...
        seq_n = self.raw_data[3]['caption'].split('_')[-1][43:-4] if self.raw_data[3]['caption'].split('_')[-1][-4] == '-' else self.raw_data[3]['caption'].split('_')[-1][43:-5]
        path = '/home/apple/ICRA2024/DCPT/vis/test-nat2021-{}'.format(seq_n)
        try:
            os.mkdir(path)
        except:
            pass
        legendpic = cv2.imread("legend_resize.jpg")
        write_img = np.concatenate((write_img, legendpic), axis=0)
        cv2.putText(write_img, seq_n+self.raw_data[3]['caption'].split('_')[-1][43+len(seq_n)]+'#'+self.raw_data[3]['caption'].split('_')[-1][44+len(seq_n):], (30,30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (2,255,250), 2)
        cv2.imwrite(os.path.join(path, self.raw_data[3]['caption'].split('_')[-1] + '.jpg'), write_img)
        self.videowriter.write(write_img)
        if(self.raw_data[3]['caption'].split('_')[-1][-4:] == '1367'):
            self.videowriter.release()
            print("release")

        disp_image = cv2.resize(disp_image,None,fx=0.001,fy=0.001)
